...
describe("redactUrl", () => {
	...
	it("redacts query parameter values", () => {
		expect(redactUrl("https://example.com/path?token=abc123")).toBe("https://example.com/path?token=[redacted]");
	});
	...
	it("handles fragments after query string", () => {
		...
		expect(redactUrl("https://example.com/path?q=1#section")).toBe("https://example.com/path?q=[redacted]#section");
	});
});
...
describe("Log redaction (6.3)", () => {
	...
	it("redacts query parameters in logger calls by default", async () => {
		...
		const urlsInLogs = logs.filter((l) => typeof l.data?.url === "string").map((l) => l.data.url as string);
